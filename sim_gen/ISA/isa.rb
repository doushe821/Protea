      ...
      def find_max_reg(regfiles)
        ...
        regfiles.each do |regfile|
          regfile[:regs].each do |reg|
            max_reg_size = reg[:size] if reg[:size] > max_reg_size
          end
        end
        max_reg_size
      end
      ...
      def find_max_operands(instructions)
        ...
        instructions.each do |insn|
          ...
          insn[:map][:tree].each do |node|
            next unless node[:name] == :new_var && !node[:attrs].nil? && node[:attrs].include?(:op)

            operands_count += 1
            if Utility.get_type(node[:oprnds][0][:type]).bitsize > max_size
              max_size = Utility.get_type(node[:oprnds][0][:type]).bitsize
            end
          end
      ...
      def get_addr_type(instructions)
        instructions.each do |insn|
          insn[:code][:tree].each do |node|
            return node[:oprnds][0][:type] if %i[writeMem readMem].include?(node[:name])
          end
        end
      end
